...
#            imgs=np.expand_dims(imgs,axis=0)
    ...
    def get_unique_trajectories(self,cell_inds=None,verbose=False,extra_depth=None):
        ...
        while n_untracked >0:
            indc=cell_inds_all[-1]
            cell_traj=self.get_cell_trajectory(indc) # Cell indices of a trajectory tracked across frames
						# Get to know whether cell indices along a trajectory (cell_traj) correspond to an unique trajectory
    ...
    def get_dx_alpha(self,cell_inds=None,thbins=None,nth=10,rcut=40.):
        ...
        paircosth=paircosth/np.sum(paircosth)
        return thbins,paircosth
    # Get trajectory segments in a sliding window of a given snippet length 
    def get_traj_segments(self,seg_length):
        ntraj = len(self.trajectories)
        traj_segSet = np.zeros((0, seg_length)).astype(int)
        for itraj in range(ntraj):
            cell_traj = self.trajectories[itraj] # select a "whole" single-cell trajectory
            traj_len = cell_traj.size # Length of the selected trajectory
            # Extract trajectory snippets of a given length (seg_length) from the whole single-cell trajectories (one by one)
    ...
    def prepare_cell_features(self,apply_znorm=True):
        ...
        if hasattr(self,"cellborder_fmsks"):
            ...
            x1fcb=self.featBoundaryCB(msk,fmsk) # Cell-cell interactions features
            ncb=x1fcb.size
        else:
            cbfeat=False
            print('no cell foreground masks, skipping cell border featurization')
        ng=x1fg.size # Number of Zernike features
        nh=x1fh.size # Number of Haralick features
        nb=x1fb.size # Number of boundary features --> shape
...
#        self.Xtraj=self.Xd[:,0:neigen]
    ...
    def get_transitionMatrixDeeptime(self, x0, x1, n_clusters):
        ...
        Mt = Cm.copy()
        sM = np.sum(Mt, 1) # Sum all elements of each row
     
        for iR in range(n_clusters):
            if sM[iR] > 0:
               Mt[iR, :] /= sM[iR] # Normalize to make sure each row should be = 1.0 
            elif sM[iR] == 0.0:
               Mt[iR, iR] = 1.0
