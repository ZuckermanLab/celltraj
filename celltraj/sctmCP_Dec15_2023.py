...
class Trajectory():
        ...
        for iF in range(self.nF):
            ...
            try:
                ...
                if e:
                    ...
                    dsetName = "/images/img_%d/cmsk" % int(n_frame) # Retrieve cell masks info
                    dset=dataIn[dsetName]
                    msks[iF]=dset[:]
                    dset_foreground_msk = "/images/img_%d/fmsk" % int(n_frame) # Retrieve foreground masks info
                    dset_fmsk=dataIn[dset_foreground_msk]
                    fmsks[iF]=dset_fmsk[:]
    ...
    def get_traj_segments(self,seg_length):
        ...
        for itraj in range(ntraj):
            cell_traj = self.trajectories[itraj] # whole single-cell trajectories
            traj_len = cell_traj.size
            # Extract trajectory snippets of a given length (seg_length) from the whole single-cell trajectories
