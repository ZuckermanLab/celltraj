...
#            imgs=np.expand_dims(imgs,axis=0)
    ...
    def get_nucChanImgs(self, n_frame): #get nuclear channel images
        ...
        for iF in range(self.nF):
            ...
            try:
                ...
                sys.stdout.write('error in '+fileName+str(sys.exc_info()[0])+'\n')
        indimages=np.where(e_images>0)
        # Check if nucImgs is empty before accessing
        if np.array(nucImgs).size == 0:
           raise ValueError(f"No nuclear image is found for frame {n_frame}")
    ...
    def get_imageSet(self,start_frame,end_frame):
        sys.stdout.write('getting reporter image and cyto-mask frame: '+str(start_frame)+'...\n')
        ...
        for iS in range(start_frame+1,end_frame+1):
            sys.stdout.write('getting reporter image and cyto-mask frame: '+str(iS)+'...\n')
    ...
    def get_nucImgSet(self, start_frame, end_frame):
        sys.stdout.write('getting nuclear images frame: '+str(start_frame)+'...\n')
        self.get_nucChanImgs(start_frame)
        self.nucImgSet = self.nucImgs.copy()
        for iS in range(start_frame+1, end_frame+1):
            sys.stdout.write('getting nuclear images frame: '+str(iS)+'...\n')
            self.get_nucChanImgs(iS)  
    ...
    def replace_garbage_image_data(self, frameID, ncellsLastFrame, ncellsCurrFrame):
        ...
        if diffCells > 50:
          ...
          self.nucImgSet[frameID, :, :] = self.nucImgSet[frameID-1, :, :]
    ...
    def get_cell_images(self,indcells=None):
        ...
        # Register images (all frames) of a well & field of view while calculating 
        ...
        #correlations between cell-cycle reporter and nuclear channel images
        #sys.stdout.write('stack has not been trans registered: calling get_imageSet_trans()\n')
        #self.get_imageSet_trans()
...
#        self.Xtraj=self.Xd[:,0:neigen]
    ...
    def get_stack_trans(self,mskSet):
        ...
        for iS in range(1,nframes):
            print("Registering images between frames ",iS-1," and ",iS)
            msk0=mskSet[iS-1,:,:]
