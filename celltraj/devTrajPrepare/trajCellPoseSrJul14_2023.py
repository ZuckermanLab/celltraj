...
class cellPoseTraj():
    ...
    def get_image_data(self, n_frame):
        ...
        # Compute foreground masks from whole cell masks that is where no background 
        fmsks = cmsks > 0
        self.fmsks = fmsks 
        self.cmsks = cmsks
        self.imgs = imgs
    ...
    def get_cell_data(self):
        ...
        for im in range(nImg):
            ...
            if(im != 0) ncells = replace_garbage_image_data(im, ncellsLastFrame, ncellsCurrFrame) 
    ...
    def featBoundaryCB(self, cmsk, ncomp = 15, center = None, nth = 256):
        try:
            nx = int(np.sqrt(cmsk.size))
            cmsk = cmsk.reshape(nx,nx)
            ccborder, csborder = self.get_cc_cs_border(cmsk)
            if center is None:
    ...
    @staticmethod
    def get_border_profile(cmskcell, bordersize=10): # This function isn't being used in this script @ July 10, 2023.
