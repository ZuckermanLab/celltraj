...
class cellPoseTraj():
    ...
    def get_cell_data(self):
        ...
        nFrames = np.shape(self.cmskSet)[0]
        totalcells = 0
        ...
        msk = [self.cmskSet[im] for im in range(nFrames)]
        # Get cell blocks concurrently by exploiting JOBLIB parallelization                            
        cellblocks = Parallel(n_jobs = n_jobs, verbose = 10)(delayed(self.get_cell_blocks)(msk[i]) for i in range(nFrames))
        #cellBlk = Parallel(n_jobs = n_jobs, verbose = 10)(delayed(self.get_cell_blocks)(msk[i]) for i in range(nFrames))
        #cellblocks = []
        #for icb in cellBlk:
        #    cellblocks.extend(icb) # Collect data from all threads/processes 
        #cellblocks = cellblocks[:]
      
        for im in range(nFrames):
            #cellblocks = self.get_cell_blocks(msk[im])
    ...
    def get_lineage_bunch_overlap(self, distcut=45.0, distcutb=300.0, overlapcut=10.0, cellcut=10.0,
        ...
        if self.visual:
            plt.figure(figsize = (10, 8))
            nx = self.cmskSet.shape[1]; ny = self.cmskSet.shape[2]
            mindx = np.min(0 - self.cmskSet[:, 1])
            mindy = np.min(0 - self.cmskSet[:, 2])
            maxdx = np.max(nx - self.cmskSet[:, 1])
            maxdy = np.max(ny - self.cmskSet[:, 2])
            if pathto is None:
                pass
            else:
        ...
        sr = StackReg(StackReg.RIGID_BODY) # Stack register if it isn't already
        nimg = self.imgfileSet.size # Number of frames (TIFF images) in an image stack, this should be renamed to stackSet
        linSets = [None]*nimg
        stack_inds = np.unique(self.imgfileSet).astype(int)
        istack = 0
        sys.stdout.write('tracking '+self.fileList[0]+'\n') # One stack for now, replace '0' (in future) if more than one
        inds = np.where(self.imgfileSet == 0)
        inds = inds[0]
        cmskSet = self.cmskSet[inds, :, :]
        nframes = cmskSet.shape[0]
        indt0 = np.where(self.cells_indcmskSet == inds[0])[0]
        lin0 = np.arange(indt0.size).astype(int)
        linSets[inds[0]] = lin0.copy()
        #sys.exit(0)
        ...
        linCell = Parallel(n_jobs = n_jobs, verbose = 10)(delayed(self.lineage_bunch_overlap_jlp)(im, istack, bunchcut, distcut, 
                                                          distcutb, overlapcut, cellcut, pathto, clustervisual, sr, cmskSet,
                                                          inds, indt0, lin0, linSets) for im in range(nframes - 1))
        linSet = []
        for ic in linCell:
            linSet.extend(ic) # combine the results from all processes
    ...
    def prepare_cell_features(self, apply_znorm=True):
        ...
        for ic in range(self.ncells):
            Xf[ic] = self.get_cellFeat(ic, Xf1[ic], cbfeat)
...
#        self.Xtraj=self.Xd[:,0:neigen]
    ...
    def transform_image(x1, t):
        if x1.ndim == 1:
            ...
            x1 = x1.reshape(nx, nx)
            print("Reshaping the 1 dimensional images/masks")
        nx = x1.shape[0]
        ny = x1.shape[1]
        centerx = nx/2
        centery = ny/2
    ...
    def stack_translation(self, cmskSet, iS):
        
        cmskSet_copy = cmskSet.copy()
        
        msk0 = cmskSet_copy(iS-1, :, :)
        ...
        msk1t = self.transform_image(msk1, t_global) # Transformed Images/Masks -- Renamed by JS
        t_local = self.get_minT(msk0, msk1t, nt = self.ntrans, dt = self.maxtrans)
        tSet[:] = t_global + t_local
        sys.stdout.write('transx: '+str(tSet[1])+' transy: '+str(tSet[2])+'\n')
        if self.visual:
            plt.clf()
            msk1 = cmskSet_copy(iS-1, :, :)
            msk1t = self.transform_image(msk1, tSet[iS])
            msk0 = cmskSet_copy(iS, :, :)
            plt.subplot(1, 2, 1)
    ...
    def get_stack_trans(self, cmskSet):
        ...
        tSetOut = Parallel(n_jobs = n_jobs, verbose = 10)(delayed(self.stack_translation)(cmskSet,
                                                          i) for i in range(1, nframes))
        for k in tSetOut:
          for l, c in enumerate(k):
            tSet[l].extend(c) # combine the results from all processes/workers
        
        # for fid in range(1, nframes):
        #    tSet[fid] = self.stack_translation(cmskSet[fid-1, :, :], cmskSet[fid, :, :], fid)
