...
# Save cell cycle reporter and nuclear channel images along with cytoplasmic masks
def save_frame_h5_CCR(filename, frame, img=None, nuc=None, cmsk=None, overwrite=False, timestamp=None):
    iS = frame
    if timestamp is None:
        timestamp = float(frame)
    ...
    with h5py.File(filename, 'a') as f:
         ...
         if nuc is not None:
            dsetName = "/images/img_%d/nuc" % int(iS)
            try:
                dset = f.create_dataset(dsetName, np.shape(nuc))
                dset[:] = nuc
                dset.attrs['time'] = timestamp
            except:
                sys.stdout.write('nuc '+str(iS)+' exists\n')
...
# Save cell cycle reporter along with nuclear images and their masks
def save_frame_h5CCR_nmsks(filename, frame, img=None, nmsk=None, nuc=None, overwrite=False, timestamp=None):
    iS = frame
    if timestamp is None:
        timestamp = float(frame)
    ...
    with h5py.File(filename, 'a') as f:
         ...
         if nmsk is not None:
            dsetName = "/images/img_%d/nmsk" % int(iS)
            try:
                dset = f.create_dataset(dsetName, np.shape(nmsk))
                dset[:] = nmsk
                dset.attrs['time'] = timestamp
            except:
                sys.stdout.write('nmsk '+str(iS)+' exists\n')
         ...
         if nuc is not None:
            dsetName = "/images/img_%d/nucImg" % int(iS)
            try:
                dset = f.create_dataset(dsetName, np.shape(nuc))
                dset[:] = nuc
                dset.attrs['time'] = timestamp
